...
class InMemoryTaskManager(TaskManager):
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        try:
            ...
            # In a real scenario, this is where you would trigger actual task execution.
            # For InMemoryTaskManager, we'll just acknowledge receipt.
            # We could also immediately send a SUBMITTED event via enqueue_events_for_sse
            # if there were subscribers, but on_send_task is non-streaming.
            logger.info(f'Task {task.id} upserted with status {task.status.state}')
        except Exception as e:
            logger.error(f'Error in on_send_task while upserting: {e}', exc_info=True)
            return SendTaskResponse(
                id=request.id,
                error=InternalError(message=f'Failed to send task: {e}'),
            )
    ...
    async def on_send_task_subscribe(
        ...
        try:
            task = await self.upsert_task(task_send_params)
            logger.info(f'Task {task.id} upserted for subscription with status {task.status.state}')

            sse_event_queue = await self.setup_sse_consumer(task_id=task.id)

            # Send initial SUBMITTED event
            initial_event = TaskStatusUpdateEvent(
                taskId=task.id,
                status=TaskStatus(state=TaskState.SUBMITTED, message="Task submitted and waiting for processing."),
                artifacts=[], # No artifacts initially
                final=False
            )
            await self.enqueue_events_for_sse(task_id=task.id, task_update_event=initial_event)
            logger.info(f'Enqueued initial SUBMITTED event for task {task.id}')

            # In a real system, another component would now:
            # 1. Pick up the task.
            # 2. Execute it.
            # 3. Use `update_store` and `enqueue_events_for_sse` to send further updates
            #    (e.g., PROCESSING, COMPLETED with artifacts, or FAILED).
            # For this InMemoryTaskManager, it will just stream the SUBMITTED event
            # and then wait for external updates or client disconnect.

        except ValueError as ve: # Catch specific errors like task not found for resubscription
            logger.error(f'ValueError in on_send_task_subscribe: {ve}', exc_info=True)
            return JSONRPCResponse(id=request.id, error=InternalError(message=str(ve)))
        except Exception as e:
            logger.error(f'Error in on_send_task_subscribe: {e}', exc_info=True)
            # For streaming, we might yield an error event if the stream has already started,
            # but if setup fails, returning a JSONRPCResponse error is appropriate.
            return JSONRPCResponse(
                id=request.id,
                error=InternalError(message=f'Failed to subscribe to task: {e}'),
            )

        return self.dequeue_events_for_sse(
            request_id=request.id, task_id=task.id, sse_event_queue=sse_event_queue
        )
